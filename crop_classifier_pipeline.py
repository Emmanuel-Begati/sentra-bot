...
def load_and_split_dataset():
    ...
    _, val_dataset_proper, test_dataset_proper = random_split(
        val_test_dataset, [train_size, val_size, test_size],
        generator=torch.Generator().manual_seed(42)
    )
    
    return train_dataset, val_dataset_proper, test_dataset_proper, class_names, num_classes
...
def predict_random_images(model, test_dataset, class_names, num_images=5):
    ...
    for i, idx in enumerate(indices):
        ...
        if predicted.item() == label:
            plt.title(f'True: {true_class} | Predicted: {predicted_class}', color='green')
        else:
...
def main():
    ...
    # 8. Save final model
    torch.save(model.state_dict(), os.path.join(Config.output_dir, f"{Config.model_type}_final.pth"))
    print(f"Final model saved to {os.path.join(Config.output_dir, f'{Config.model_type}_final.pth')}")
    ...
    print(f"Model report saved to {os.path.join(Config.results_dir, 'model_report.md')}")
